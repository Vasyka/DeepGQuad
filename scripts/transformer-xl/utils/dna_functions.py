...
class Genome:
    ...
    def slice_genome(self, fractions=0.2, at_idx=None):
        ...
        if fractions == [1]:
            ...
            idxs.append(0)
        print('Full dataset size:',len(self))
        if fractions is None:
            ...
            for frac in fractions:
                ...
                if new_idx > len(self):
                    new_idx = len(self)
                idxs.append(new_idx)
        parts.append(GenomePart(self, idxs[-1], len(self)))
    ...
    def log_metrics(self, y_true, y_hat, key=None):
        ...
        if self.log_auc:
            ...
            for i in range(len(y_hat[0])):
                if y_true_hot[:, i].sum() != 0:
                    try:
                        local_auc = roc_auc_score(y_true_hot[:, i], y_hat[:, i])
                    except ValueError:
                        local_auc = 0
                    auc.append(local_auc)
        ...
        if self.log_p_r:
            ...
            for i in range(len(y_hat[0])):
                if y_true_hot[:, i].sum() != 0:
                    try:
                        local_p_r = average_precision_score(y_true_hot[:, i], y_hat[:, i])
                    except ValueError:
                        local_p_r = 0
                    p_r.append(local_p_r)
